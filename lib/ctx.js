...
var Ctx = function (value, options) {
	this.value = value;
	options = options || {};
	this.stopOnFirstError = options.stopOnFirstError === undefined ? true : options.stopOnFirstError;
	this.needValidatorInfo = !!options.needValidatorInfo;
	this.needValidator = !!options.needValidator;
	this.needMessage = !!options.needMessage;
...
Ctx.prototype.createValidationError = function (validator, subErrorCode) {
	var result = {
		path: this.getPath(),
...
Ctx.prototype.validationErrorInternal = function (validator, subErrorCode) {
	if (!this.isNoError()) {
		var error = this.createValidationError(validator, subErrorCode);
		if (this.isWarningMode()) {
